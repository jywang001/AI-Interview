...
真实材料解析、模型分析、确认版 CandidateBrief、六阶段动态追问和实时 Coach 报告已经接通；PDF 仅覆盖文本型文件，不做 OCR。旧版重练页面仍为 Demo。当前框架不实现实时流式语音、数字人视频、多面试官、登录支付、长期历史或录取概率。算法环节产品形态永久采用 Hot 100 独立思考加口述讲解，不提供代码编辑器，也不在服务端执行用户代码。
